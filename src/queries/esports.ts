...
export const listMatchesFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const cacheKey = `esports:matches:${JSON.stringify(data)}`;
    const ttl = data.status === 'live' ? CACHE_TTL.ESPORTS_LIVE : CACHE_TTL.ESPORTS_STATIC;
...
export const listTeamsFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    return await getCachedData(
      CACHE_KEYS.ESPORTS_TEAMS(data.game),
      async () => {
        const db = await getDB();
...
export const listPlayersFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    return await getCachedData(
      CACHE_KEYS.ESPORTS_PLAYERS(data.game),
      async () => {
        const db = await getDB();
